  ...
  export default {
    ...
    methods: {
      input(index) {
        ...
        this.guesslist[index].answer = this.guesslist[index].answer.replace(reg, () => {
          Notice.destroy();
          Notice.warning({
            desc: this.guesslist[index].lan === 'cn' ? '请输入汉字' : '请输入英文字母'
          });
          return '';
        });
      },
      ...
      placeholder(index, lan) {
        return this.guesslist[index].lan === 'en' ? '请在此输入不超过15个的字母' : '请在此输入不超过15个的汉字';
      },
      // 保存
      getQuestionData(urlSnippet) {
        ...
        this.guesslist.forEach((item, index) => {
          let res = this.$refs.descRes[index].getResource();
          let obj = {
            resource: {
              type: res.type,
          ...
          guesslist.push(obj);
          _gursslist.push(Object.assign({}, obj, {
            resource: {
              type: res.type,
              cssStyle: res.cssStyle,
              src: res.src
