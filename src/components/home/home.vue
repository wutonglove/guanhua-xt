<template>
  <div class="q_main">
    <div class="q_list" v-for="(list,i) in questionList" :key="i">
      <div class="title">
        <Icon :type="list.icon" size="20" style="margin:0 3px"></Icon>
        <span class="text">{{list.title}}</span>
      </div>
      <div class="q_item_wrap">
        <router-link class="q_item" v-for="(item,index) in list.content" :key="index" :to="`${item.href}?${params}`"  v-if="!/^--/.test(item.name)">
