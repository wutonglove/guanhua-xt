...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      drawEnd() {
        if (this.curG) {
          let offset = Math.abs(this.curG.interval[0].x - this.curG.interval[1].x);
      ...
      getAnswer() {
        ...
        for (let m = 0; m < arr1.length; m++) {
          for (let n = 0; n < arr1[m].length; n++) {
            if (arr1[m][n].num !== arr2[m][n].num || arr1[m][n].include !== arr2[m][n].include) {
              return false;
            }
          }
        }
